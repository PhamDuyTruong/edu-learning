...
import {approveCoursePending, disapproveCourse} from '../../actions/UserManageAction'
...
const UserDetail = () => {
  ...
  const {error, success, loading, coursesPendingList, selectedUser, coursesApprovedList, coursesNoneEnrollList,  avatarIndex} = useSelector((state) => state.userManage);
  ...
  const onApproveCoursePending = (courseId, selectedUser) =>{
     dispatch(approveCoursePending(courseId, selectedUser))
  }

  const onDisapproveCourse = (courseId, selectedUser) =>{
      dispatch(disapproveCourse(courseId, selectedUser))
  }
  ...
  return (
    ...
    >
      ...
      <Box flexGrow={1} alignSelf="flex-start" minWidth={250} m={2}>
        <AddUser selectedUser={selectedUser} preview={true} />
      </Box>
    </Box>
  </Card>
  )
